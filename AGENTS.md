...
This plugin was designed by asking Codex what it needs, then building that.
Codex is the primary stakeholder. The human's role is to understand what Codex
...
- When evaluating any idea or proposed change: the first question is "does this
  reduce Codex's friction?" If the honest answer is no, reject it — regardless
...
Read `design/agent-perspective.md` now — it contains Codex's friction points,
...
What Codex can decide and act on without checking:
- Commits at logical checkpoints (autonomous by default)
- Bug fixes Codex identified itself
- Documentation and design doc updates (`design/`, `AGENTS.md`, `WORKING_AGREEMENT.md`)
...
- `design/agent-perspective.md` — Codex's friction points; the "why" behind what gets built
